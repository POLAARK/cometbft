...
func TestMempoolNoCacheOverflow(t *testing.T) {
	...
	for i := 1; i <= mp.config.CacheSize; i++ {
		_, err = mp.CheckTx(kvstore.NewTxFromID(i), "", nil)
		require.NoError(t, err)
	}
	err = mp.FlushAppConn()
	require.NoError(t, err)
...
func TestMempoolRemoteAppConcurrency(t *testing.T) {
	...
	for i := 0; i < n; i++ {
		...
		mp.CheckTx(tx, "", nil) //nolint: errcheck // will error
	}
...
func TestMempoolConcurrentCheckTxAndUpdate(t *testing.T) {
	...
	for h := 1; h <= maxHeight; h++ {
		_, err := mp.CheckTx(kvstore.NewTxFromID(h), "", nil)
		require.NoError(t, err)
	}
